...
class HeavyT_MLP(nn.Module):
    def __init__(self, input_dim: int = 2, hidden_dim: int = 64):
        ...
        # Similar structure to FMnet but with basicTTF instead of final ELU
        ...
        self.ht_activation = basicTTF(dim=input_dim)
    ...
    def forward(self, x: Tensor, t: Tensor) -> Tensor:
        ...
        # Expand time tensor to match batch size
        t = t.expand(x.shape[0], 1)

        # Concatenate time with input like FMnet (t, x)
        h = torch.cat((t, x), -1)

        # Pass through main layers (up to the last ELU)
        h = self.main_layers(h)

        # Apply final linear layer
        output = self.final_linear(h)

        # Apply basicTTF activation instead of no activation
        output = self.ht_activation(output, t)
